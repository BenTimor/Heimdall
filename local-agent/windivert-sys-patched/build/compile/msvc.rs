...
pub fn lib() {
    ...
    // Rename memcpy/memset to private symbols so they don't shadow the CRT
    // versions when statically linked. WinDivert defines these for its DLL
    // build (/NODEFAULTLIB), but in a static library they leak into the
    // host binary and cause infinite recursion (MSVC optimizes the byte-by-
    // byte loop back into `call memcpy`).
    build.define("memcpy", "windivert_memcpy");
    build.define("memset", "windivert_memset");
